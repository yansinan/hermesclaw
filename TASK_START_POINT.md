...
工作目录与仓库
- 仓库路径：/opt/data/wechat-route
- 已在 ~/.hermes/skills 创建软链：~/.hermes/skills/wechat-route -> /opt/data/wechat-route
...
- .env：已更新，STATE_FILE=/opt/data/wechat-route/router_state.json, LOG_FILE=/opt/data/wechat-route/hermesclaw.log
...
- feature-container-hermes-side.bundle：已存在于 /opt/data/wechat-route，用于离线推送

已完成的安装/修改里程碑
- 仓库已从 /opt/data/hermesclaw 重命名为 /opt/data/wechat-route 并更新相关路径引用
...
- 恢复旧路径：如果需要把 /opt/data/hermesclaw 恢复名，使用 mv /opt/data/wechat-route /opt/data/hermesclaw
